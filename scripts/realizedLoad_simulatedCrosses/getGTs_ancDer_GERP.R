##### Script is to print the genotypes for all individuals in each population. Genotypes are recoded in [0, 1, 2] format relative to ancestral-derived definitions, indicating whether there are 0, 1, or 2 copies of the derived allele. This script processes the GERP analysis. #####

# Author use only: GitHub version derived from version 5 of KL14 GERP script 01b_getGTs_KL14_ancestralGERP_unprocessedPopsOnly.R, 2024/09/10;
...
system("mkdir genotypes_GERP_byPop", intern = F);
...
foreach (k=1:length(allPops_min3ind)) %dopar% {
  ...
  for (i in 1:length(allSplits)) {
    ...
    cur_df[cur_df == "./."] <- NA;
    #
    write.table(cur_df[ , c("chr", "pos", "N", "S", cur_samples)], paste0("tmp/", cur_pop, "_", allSplits[i], ".txt"), sep = "\t", row.names = F, col.names = T, quote = F);
  ...
  system(paste0("cat tmp/", cur_pop, "_aa.txt | head -1 > genotypes_GERP_byPop/genotypes_derAnc_GERP_", cur_pop, ".txt"));
  system(paste0("tail -n +2 -q tmp/", cur_pop, "* >> genotypes_GERP_byPop/genotypes_derAnc_GERP_", cur_pop, ".txt"));
