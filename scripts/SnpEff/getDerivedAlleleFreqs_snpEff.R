##### For the SnpEff analysis, this script will calculate the derived allele frequencies at each site (a) globally, (b) for each population, and (c) for each individual. The derived allele frequencies are calculated from called genotypes in a VCF file. There are several prerequisites (that also show the input file organization) described below. #####

# Author use only: GitHub version derived from KL14 script getDerivedAlleleFreqs_snpEff_KL14_KRI_ancestralGERP.R, 2024/09/09;
...
system(paste0("mkdir derivedAlleleFreqs_snpEff_byPop"), intern = F);
system(paste0("mkdir derivedAlleleFreqs_snpEff_byInd"), intern = F);
...
pop_sampleSize <- aggregate(samples$individual, by = list(samples$pop), FUN = length);
allPops_min3ind <- pop_sampleSize$Group.1[pop_sampleSize$x >= 3];
...
for (k in 1:length(batches)) {
  ...
  foreach (i=batches[[k]]) %dopar% {
    ...
    cur_geno <- read.table(paste0("genotypes_splitFile/", allSplits[i]), header = F, sep = "\t", stringsAsFactors = F);
    colnames(cur_geno) <- c("chr", "pos", "ref", "alt", samples$individual);
    
    cur_df <- merge(cur_anc[ , c("chr", "pos", "ref", "alt", "ancestral", "derived", "annotation", "putative_impact", "gene_name")], cur_geno, by = c("chr", "pos", "ref", "alt"), sort = F);
    ...
    #
    write.table(cur_df[ , c("chr", "pos", "annotation", "putative_impact", "gene_name", "n_homAnc", "n_het", "n_homDer")], paste0("tmp/freqs_", allSplits[i], ".txt"), sep = "\t", row.names = F, col.names = T, quote = F);
...
system(paste0("cat tmp/freqs_aa.txt | head -1 > derivedAlleleFreqs_snpEff_allPops.txt"));
system(paste0("tail -n +2 -q tmp/freqs* >> derivedAlleleFreqs_snpEff_allPops.txt"));
...
foreach (k=1:length(allPops)) %dopar% {
  ...
  for (i in 1:length(allSplits)) {
    ...
    #cur_df$freq_derived <- round((cur_df$n_het + 2*cur_df$n_homDer) / (2*cur_df$n_total), 4);
    #
    write.table(cur_df[ , c("chr", "pos", "annotation", "putative_impact", "gene_name", "n_homAnc", "n_het", "n_homDer")], paste0("tmp/", cur_pop, "_", allSplits[i], ".txt"), sep = "\t", row.names = F, col.names = T, quote = F);
  ...
  system(paste0("cat tmp/", cur_pop, "_aa.txt | head -1 > derivedAlleleFreqs_snpEff_byPop/derivedAlleleFreqs_snpEff_", cur_pop, ".txt"));
  system(paste0("tail -n +2 -q tmp/", cur_pop, "* >> derivedAlleleFreqs_snpEff_byPop/derivedAlleleFreqs_snpEff_", cur_pop, ".txt"));
...
registerDoParallel(cores=length(batches));
...
for (h in 1:length(batches)) {
  ...
  foreach (k=batches[[h]]) %dopar% {
    ...
    system(paste0("cat tmp/", cur_ind, "_aa.txt | head -1 > derivedAlleleFreqs_snpEff_byInd/derivedAlleleFreqs_snpEff_", cur_ind, ".txt"));
    system(paste0("tail -n +2 -q tmp/", cur_ind, "* >> derivedAlleleFreqs_snpEff_byInd/derivedAlleleFreqs_snpEff_", cur_ind, ".txt"));
